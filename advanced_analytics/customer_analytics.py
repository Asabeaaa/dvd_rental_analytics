...
def analyse_segmentation(payment: pd.DataFrame) -> None:
...
def analyse_lifetime_value(customer: pd.DataFrame, payment: pd.DataFrame, rental: pd.DataFrame) -> None:
...
def analyse_churn_risk(customer: pd.DataFrame, rental: pd.DataFrame) -> None:
    last_rental = rental.groupby("customer_id")[
        "rental_date"].max().reset_index()
    last_rental.columns = ["customer_id", "last_rental_date"]
...
def analyse_behavioral_patterns(customer: pd.DataFrame, rental: pd.DataFrame, payment: pd.DataFrame) -> None:
